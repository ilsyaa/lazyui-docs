...
<div x-data="{
    ...
    init() {
        this.$nextTick(() => {
            if (this.$refs.originalInput.value) {
                this._selected = this.$refs.originalInput.value.split(',').filter(item => item.trim() !== '');
            }
        })
    ...
    focusNextOption() {
        ...
        const index = options.indexOf(this._focus);
        this._focus = options[(index + 1) % options.length];
    },
    ...
    focusPreviousOption() {
        ...
        if (options.length === 0) return;
        const index = options.indexOf(this._focus);
        this._focus = options[(index - 1 + options.length) % options.length];
    },
}" class="relative w-full" x-on:click.outside="close()" x-on:keydown.escape.prevent="close()">
    <div class="w-full">
        <input type="hidden" x-ref="originalInput" {{ $attributes->only(['x-model', 'wire:model', 'name', 'value', '@input.debounce.500ms']) }}>
        ...
        >
            <template x-for="value in _selected" :key="value">
                <div class="inline-flex items-center gap-1 px-2 py-1 font-medium text-xs bg-cat-300/30 dark:bg-cat-700/30 text-cat-800 dark:text-white rounded-md select-none">
                    ...
                    <button type="button" class="text-cat-500 cursor-pointer outline-transparent rounded-full" x-on:click.stop="removeSelected(value)">
                        <svg class="size-3.5" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512">
                            <defs>
                                <style>
                            ...
                            <path fill="currentColor" class="fa-primary" d="M209 175c-9.4-9.4-24.6-9.4-33.9 0s-9.4 24.6 0 33.9l47 47-47 47c-9.4 9.4-9.4 24.6 0 33.9s24.6 9.4 33.9 0l47-47 47 47c9.4 9.4 24.6 9.4 33.9 0s9.4-24.6 0-33.9l-47-47 47-47c9.4-9.4 9.4-24.6 0-33.9s-24.6-9.4-33.9 0l-47 47-47-47z" />
                        </svg>
                    </button>
                </div>
            </template>
            <input
                ...
                x-on:keydown.down.prevent="focusNextOption()"
                x-on:keydown.up.prevent="focusPreviousOption()"
                x-on:keydown.enter.prevent="_focus ? toggleSelected(_focus) : setSelected(_input)"
            >
        </div>
        <div x-ref="dropdown" x-show="_isOpen && getFilteredOptions().length > 0" x-cloak x-anchor.bottom.offset.8="$refs.toggle" role="listbox" class="z-[61] bg-white/90 rounded-md dark:bg-cat-800/90 lazy-gradient backdrop-blur w-full shadow">
            <ul class="[&::-webkit-scrollbar]:!w-1 overflow-y-auto max-h-60 flex flex-col gap-y-1 p-1">
                <template x-for="value in getFilteredOptions()" :key="value">
                    <li :class="{ 'bg-cat-300/50 dark:bg-cat-700/30': _focus === value, 'bg-cat-300/30 dark:bg-cat-700/15': _selected.includes(value) }" class="px-2.5 py-2 rounded-lg text-sm w-full cursor-pointer hover:bg-cat-300/30 dark:hover:bg-cat-700/15 select-none" x-on:click="toggleSelected(value);" role="option">
                        <div class="flex items-center gap-2">
                            @if ($checkbox)
                                <input type="checkbox" x-bind:checked="_selected.includes(value)" class="peer pointer-events-none block relative size-4 m-0.5 bg-transparent border-cat-400 dark:border-cat-500 rounded !ring-offset-transparent focus:ring-transparent disabled:cursor-not-allowed disabled:opacity-50 before:absolute before:left-1/2 before:top-1/2 before:-translate-x-1/2 before:-translate-y-1/2 before:rounded-full before:size-[1.9rem] disabled:before:bg-transparent checked:bg-accent-500 checked:border-accent-500 indeterminate:!border-accent-500 hover:before:bg-accent-500/10 before:focus-visible:bg-accent-500/10" />
                            @endif
                            <span x-text="value"></span>
                        </div>
                    </li>
