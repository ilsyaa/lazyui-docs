...
<div x-data="{ open: false, childElementOpen: false }" x-cloak x-show="(selectedItems.length > 0 || hideBulkActionsWhenEmpty == false)" class="w-full md:w-auto mb-4 md:mb-0">
    <div class="relative inline-block text-left z-10 w-full md:w-auto">
        <button {{ $attributes->merge($this->getBulkActionsButtonAttributes)->class(['relative inline-flex items-center justify-center whitespace-nowrap rounded-md text-sm font-medium transition-colors cursor-pointer focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-ring disabled:pointer-events-none disabled:opacity-50', 'h-9 w-full px-4 py-2', 'bg-cat-900 dark:bg-white text-white dark:text-cat-900 shadow hover:bg-cat-800/90 dark:hover:bg-white/90'])->except(['default', 'default-styling', 'default-colors']) }} type="button" id="{{ $tableName }}-bulkActionsDropdown" x-on:click="open = !open" aria-haspopup="true" aria-expanded="false">
        ...
        <div x-on:click.away="if (!childElementOpen) { open = false }" @keydown.window.escape="if (!childElementOpen) { open = false }" x-cloak x-show="open" x-transition:enter="transition ease-out duration-100" x-transition:enter-start="transform opacity-0 scale-95" x-transition:enter-end="transform opacity-100 scale-100" x-transition:leave="transition ease-in duration-75" x-transition:leave-start="transform opacity-100 scale-100" x-transition:leave-end="transform opacity-0 scale-95" class="origin-top-right absolute right-0 mt-2 w-full md:w-48 focus:outline-none z-50">
